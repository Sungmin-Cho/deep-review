 ...
 *
 * Identity contract: producer === 'deep-review', artifact_kind === 'recurring-findings',
 * schema.name === artifact_kind. unwrapEnvelope() enforces all three (handoff §4
 * round-4 "envelope identity guards"). deep-review currently emits a single artifact
 * kind; the ALLOWED_ARTIFACT_KINDS Set is parameterised for future sidecar-JSON
 * additions (review-report, etc. — out of scope for v1.4.0).
 */
...
const PLUGIN_NAME = 'deep-review';
const ALLOWED_ARTIFACT_KINDS = Object.freeze(new Set(['recurring-findings']));
...
function loadProducerVersion() {
  ...
  // contexts may run helpers from arbitrary directories (handoff §4
  // literal-cwd-resolve lesson — deep-docs round-1).
 ...
 *
 * opts:
 *   artifactKind     'recurring-findings' (required, future: sidecar kinds)
 *   payload          plain object (required, non-null, non-array)
 *   parentRunId      optional ULID — cross-plugin chain (e.g. session-receipt.run_id)
...
function wrapEnvelope(opts) {
  ...
  if (typeof opts.parentRunId === 'string' && opts.parentRunId.length > 0) {
    ...
    // validation. A typo on --parent-run-id would otherwise mint an envelope
    // that fails strict consumers but succeeds the helper.
 ...
 *
 * Identity is checked on producer === 'deep-review', artifact_kind === expectedKind,
...
function unwrapEnvelope(obj, expectedKind) {
  ...
  if (
    ...
    process.stderr.write(
      `[deep-review/envelope] identity mismatch: expected producer=${PLUGIN_NAME} kind=${expectedKind}, got ${JSON.stringify(id)}\n`,
    );
    return null;
  }
  ...
  if (payload === null || typeof payload !== 'object' || Array.isArray(payload)) {
    ...
    process.stderr.write(
      `[deep-review/envelope] corrupt payload: expected object, got ${
