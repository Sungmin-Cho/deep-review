...
- **Grok/xAI는 기본 비활성(opt-in)입니다.** 탐지만으로 Grok을 선출하거나 dispatch하지 않으며 일반 무플래그 리뷰는 Grok/xAI에 저장소 내용을 보내지 않습니다. `--grok`(또는 명시적인 Grok 대상 routing override나 policy opt-in)을 사용해야 후보가 되고, dispatch 전에 외부 privacy preflight를 통과해야 합니다. `--codex-only`는 `--no-grok`을 추가해 문자 그대로 Codex-only 의미를 보존합니다. `--grok`은 활성화된 플랫폼(이 릴리스에서는 `linux/x64`)에서만 선택할 수 있으며, 봉쇄 헬퍼를 출하하는 태그된 릴리스 커밋에서 플러그인을 설치하고, Grok CLI가 `1.0.4` 또는 `1.0.13`이며, 런처가 네이티브 실행 파일일 때입니다. `win32/x64`는 헬퍼를 출하하지만 실측 턴 게이트를 통과할 때까지 `platform_verification_pending`으로 거부되며, macOS와 그 밖의 모든 플랫폼은 `unsupported_grok_containment`로 fail-closed입니다. 거부는 typed입니다: `missing_grok_containment_helper`, `grok_containment_helper_failed`(이 호스트에서 봉쇄를 수립할 수 없거나 `SHA256SUMS` 검사에 실패한 헬퍼), `unsupported_grok_cli_version`, `incompatible_grok_cli` (`.cmd`/PowerShell/`#!` 런처 포함). 소스 checkout에서 `npm run build:native`로 컴파일하는 것은 여전히 on-switch가 아닙니다.
